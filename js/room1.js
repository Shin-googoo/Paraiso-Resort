...
const slider_array = [document.querySelector("#small_slide_1"), document.querySelector("#small_slide_2"), document.querySelector("#small_slide_3"), document.querySelector("#small_slide_4"), document.querySelector("#small_slide_5")]
...
slider_array.forEach(function(small_slider, index) {
    slider_array[index].addEventListener("click", () => {
        ...
        if(picNum === index) {
            ...
            let pic_num = index+1;
            // 큰 사진 변경
            big_pic.src = `./image/room1_big${pic_num}.jpg`;
...
small_left.addEventListener("click", () => {
    // picNum이 0일 경우 picNum을 4로 넘겨주어 처리
    if(picNum === 0) {
        // 큰 사진 변경
        big_pic.src = `./image/room1_big5.jpg`;

        // 버튼 클릭 시 picNum을 4로 변경하며 해당 사진 활성화
        slider_array[picNum].classList.remove("active");
        slider_array[4].classList.add("active");
        picNum = 4;
    } else {
        // 큰 사진 변경
        big_pic.src = `./image/room1_big${picNum}.jpg`;
...
small_right.addEventListener("click", () => {
    // picNum이 4일 경우 picNum을 0으로 넘겨주어 처리
    if(picNum === 4) {
        // 큰 사진 변경
        big_pic.src = `./image/room1_big1.jpg`;
