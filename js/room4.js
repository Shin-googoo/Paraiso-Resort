...
const slider_array = [document.querySelector("#small_slide_1"), document.querySelector("#small_slide_2"), document.querySelector("#small_slide_3")]
...
slider_array.forEach(function(small_slider, index) {
    slider_array[index].addEventListener("click", () => {
        ...
        if(picNum === index) {
            ...
            let pic_num = index+1;
            // 큰 사진 변경
            big_pic.src = `./image/room4_big${pic_num}.jpg`;
...
small_left.addEventListener("click", () => {
    // picNum이 0일 경우 picNum을 2로 넘겨주어 처리
    if(picNum === 0) {
        // 큰 사진 변경
        big_pic.src = `./image/room4_big3.jpg`;

        // 버튼 클릭 시 picNum을 2로 변경하며 해당 사진 활성화
        slider_array[picNum].classList.remove("active");
        slider_array[2].classList.add("active");
        picNum = 2;
    } else {
        // 큰 사진 변경
        big_pic.src = `./image/room4_big${picNum}.jpg`;
...
small_right.addEventListener("click", () => {
    // picNum이 2일 경우 picNum을 0으로 넘겨주어 처리
    if(picNum === 2) {
        // 큰 사진 변경
        big_pic.src = `./image/room4_big1.jpg`;
